...
class OpenBCMP:
    ...
    # ---------- Node metric kernels ----------
    def _mmm_metrics(self, lam_i, mu_i, m_i, eps=1e-12):
        """
        M/M/m 指標（Erlang C）。返り値のキーは従来どおり:
        rho, pi0, Pwait, Lq, Wq, W, L, a, unstable
        ただし内部計算は log 領域（lgamma, log1p）で数値安定化。
        """
        import math
        import numpy as np
        ...
        # 自明/異常
        if mu_i <= 0 or m_i <= 0:
            return dict(rho=np.inf, pi0=np.nan, Pwait=np.nan,
                        Lq=np.inf, Wq=np.inf, W=np.inf, L=np.inf,
                        a=np.inf, unstable=True)

        if lam_i <= 0:
            # 到着ゼロ：待ち無し（W=1/μ、その他0）
            return dict(rho=0.0, pi0=1.0, Pwait=0.0,
                        Lq=0.0, Wq=0.0, W=1.0/mu_i, L=0.0,
                        a=0.0, unstable=False)
        ...
        # 不安定は即リターン（上位でペナルティ）
        if rho >= 1.0 - eps:
            return dict(rho=rho, pi0=0.0, Pwait=1.0,
                        Lq=np.inf, Wq=np.inf, W=np.inf, L=np.inf,
                        a=a, unstable=True)

        # --- log-sum-exp ユーティリティ（ローカル関数） ---
        def _logsumexp(log_terms):
            m = max(log_terms)
            s = sum(math.exp(t - m) for t in log_terms)
            return m + math.log(s) if s > 0.0 else -math.inf

        log_a = math.log(a)

        # 分母: Σ_{k=0}^{m-1} a^k/k!  +  a^m/(m!)*(1/(1-rho))
        log_terms = [k*log_a - math.lgamma(k+1) for k in range(m_i)]
        log_sum   = _logsumexp(log_terms)

        tail_log  = (m_i*log_a) - math.lgamma(m_i+1) - math.log1p(-rho)
        log_den   = _logsumexp([log_sum, tail_log])

        # pi0 = 1/den, Pwait = tail/den
        pi0    = math.exp(-log_den)
        Pwait  = math.exp(tail_log - log_den)

        # 待ち行列長/待ち時間/系内人数
        Lq = (rho / (1.0 - rho)) * Pwait
        Wq = Lq / lam_i
        W  = Wq + 1.0/mu_i
        L  = m_i * rho + Lq

        return dict(rho=rho, pi0=pi0, Pwait=Pwait,
                    Lq=Lq, Wq=Wq, W=W, L=L,
                    a=a, unstable=False)
